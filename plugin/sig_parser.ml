...
type pre_arg_ty = PAT_type of string | PAT_bind of pre_arg_ty
type pre_ctor_ty = pre_arg_ty list * string
type pre_signature = { types : string list; ctors : pre_ctor_ty list }

let is_ascii (c : char) : bool = Char.code c <= 127
let is_space (c : char) : bool = match c with ' ' | '\t' -> true | _ -> false
let is_end_of_line (c : char) : bool = match c with '\n' -> true | _ -> false

(**************************************************************************************)
(** *** Simple parsers. *)
(**************************************************************************************)

(** Parse any number of consecutive whitespace. *)
let spaces : unit t = skip_while is_space

(** [lex p] parses [p] than eats all following whitespace. *)
let lex (p : 'a t) : 'a t = p <* spaces
...
(** Parse an identifier using functions from the Coq implementation. This allows the same
    unicode identifiers as Coq, e.g. greek letters. It works by taking bytes from the
    input stream and successively checking if they are a valid identifier. Because utf-8
    encoded codepoints may be represented by multiple bytes we have to skip the check for
    any non-ascii bytes and remember the longest valid prefix so far. If the byte is ascii
    we check if the whole string is an identifier and if so we update the longest valid
    prefix. If not we are done and return this prefix. *)
let raw_ident : string t =
  let* id =
    lex
    @@ scan_state ("", "") (fun (s, valid_prefix) c ->
           let s' = s ^ String.make 1 c in
           if is_ascii c
           then if CLexer.is_ident s' then Some (s', s') else None
           else Some (s', valid_prefix))
  in
  check_non_empty_ident id

let ident = lex raw_ident
let arrow = lex @@ string "->"
let colon = lex @@ char ':'
let type_ = lex @@ string "Type"
let lparen = lex @@ char '('
let rparen = lex @@ char ')'
let comma = lex @@ char ','
let comment_begin = lex @@ string "--"
let bind = lex @@ string "bind"
let in_ = lex @@ string "in"

(** [parens p] wraps [p] in parentheses. *)
let parens p = lparen *> p <* rparen

(** All declarations have to be each on one line ** A comment begins with "--" and is
    either terminated by a newline of the end of input ** A blank line only contains
    optional whitespace and an optional comment. ** For a line to be finished we need
    either at least one newline (followed by more blank ** lines and maybe the end of
    input) or the end of input *)
let comment =
  comment_begin
  *> skip_while (fun c -> not (is_end_of_line c))
  *> (end_of_line <|> end_of_input)

let blank_line = spaces *> (comment <|> end_of_line)

let end_line =
  skip_many1 blank_line *> option () (spaces *> end_of_input) <|> spaces *> end_of_input
...
let arg_ty : pre_arg_ty t =
  fix @@ fun arg_ty ->
  let p1 =
    let+ ty = parens (bind *> arg_ty) in
    PAT_bind ty
  in
  let p2 =
    let+ ty = ident in
...
(** Parse a constructor type. *)
let ctor_ty : (pre_arg_ty list * string) t =
  let* args = sep_by arrow arg_ty in
  let* ret = ident in
  return (args, ret)
...
(** Parse a declaration [id : Type]. *)
let type_decl : string t =
  let* id = ident in
  let* _ = colon in
  let* ty = type_ in
  return id

(** Parse a declaration [ctor : bool -> term -> (bind term) -> term]. *)
let ctor_decl : (string * pre_ctor_ty) t =
  let* id = ident in
  let* _ = colon in
  let* ty = ctor_ty in
  return (id, ty)
...
(*let sortDecl =
  lift4 (fun s name _ _ -> `Sort (s, name)) ident (option None var_name) colon ttype*)
